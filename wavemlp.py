...
import logging
#from mmseg.models import load_checkpoint  # 改为适合您使用的框架
...
if __name__ == '__main__':
    ...
    model = WaveMLP_S(num_classes=4)
    out = model(x)
    model_info = summary(model, input_size=(1, 3, 224, 224), verbose=0, flops=True)
    # 打印 FLOPs
    print(f'FLOPs: {model_info.total_ops}')
    print(out.shape)
