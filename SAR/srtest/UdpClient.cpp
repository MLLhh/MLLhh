...
int UdpClient::dealUdpSendData()
{
    ...
    while (sequence < packetNum)
    {
        ...
        if (sequence < (packetNum-1))//不是最后一片
        {
            ...
            memcpy(piecebuff, &tMergeHdr, sizeof(MergeHdr));//将包头数据拷贝到缓冲区
            memcpy(piecebuff+sizeof(MergeHdr), (char*)sendbuff+tMergeHdr.uOffset, UDP_PAYLOAD);//将有效数据截取UDP_PAYLOAD大小拷贝到包头的后面
           //发送数据并保存长度
